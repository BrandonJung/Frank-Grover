...
entt::entity PoorSquirrel::createPoorSquirrel(vec2 position)
{
	auto entity = _ecs.create();

	// create rendering components
	std::string key = "poor_squirrel";
	ShadedMesh& resource = cache_resource(key);
	if (resource.effect.program.resource == 0)
	{
		resource = ShadedMesh();
		RenderSystem::createSprite(
			resource, textures_path("poor_squirrel.png"), "textured");
	}
	...
	_ecs.emplace<PoorSquirrel>(entity);
