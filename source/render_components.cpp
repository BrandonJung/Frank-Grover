...
void gl_compile_shader(GLuint shader)
{
	...
	if (success == GL_FALSE)
	{
		...
		std::cout << "Error compiling shader: " + std::string(log.data())
				  << std::endl;
...
template <> GLResource<FRAME_BUFFER>::~GLResource() noexcept
{
	if (resource > 0)
...
void Texture::create_from_screen(GLFWwindow const* window,
								 GLuint* depth_render_buffer_id)
{
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
							  GL_RENDERBUFFER, *depth_render_buffer_id);
...
bool Texture::is_valid() const { return texture_id != 0; }

void Effect::load_from_file(std::string vs_path, std::string fs_path)
{
	// std::cout << "Loading effect from [" << vs_path << "], [" << fs_path <<
	// "]"
	//		  << std::endl;
...
// Very, VERY simple OBJ loader adapted from
// https://github.com/opengl-tutorials/ogl tutorial 7 (modified to also read
// vertex color and omit uv and normals)
void Mesh::loadFromOBJFile(std::string obj_path)
{
	...
	if (!obj_file)
	{
		std::cout << "Could not open OBJ file " << obj_path << std::endl;
			...
			for (unsigned int i = 0; i < 3; ++i)
			{
				...
				if (!ss)
				{
					throw std::runtime_error(
						"File can't be read by our simple parser :-( Try "
	...
	{
		pos.position =
			((pos.position - min_position) / extent) - vec3(0.5f, 0.5f, 0.0f);
	}
}
...
ShadedMeshRef::ShadedMeshRef(ShadedMesh& mesh) : reference_to_cache(&mesh){};
